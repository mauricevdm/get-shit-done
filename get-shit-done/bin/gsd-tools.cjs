 ...
 *
 * Worktree Operations:
 *   worktree init                      Create .planning/worktrees/ and initialize registry.json
 *   worktree add <phase> <path>        Add worktree entry to registry
 *     --branch <branch> --slug <slug>
 *   worktree remove <phase>            Remove worktree entry from registry
 *   worktree get <phase>               Get worktree info as JSON
 *   worktree list                      List all worktrees from registry
 *   worktree status                    Compare registry with git worktree list
 *
 * Lock Operations:
 *   lock record <phase>                Record lock acquisition in registry
 *   lock clear <phase>                 Remove lock entry from registry
 *   lock check <phase>                 Check if phase is locked
...
// NOTE: Worktree, Lock, and Health commands have been extracted to:
// - ./lib/worktree.cjs (worktree and lock operations)
// - ./lib/health.cjs (health check and repair operations)
...
async function main() {
  ...
  switch (command) {
    ...
    case 'worktree': {
      const subcommand = args[1];
      if (subcommand === 'init') {
        worktreeModule.cmdWorktreeInit(cwd, output, error, raw);
      } else if (subcommand === 'add') {
        const branchIdx = args.indexOf('--branch');
        const slugIdx = args.indexOf('--slug');
        worktreeModule.cmdWorktreeAdd(cwd, args[2], args[3], {
          branch: branchIdx !== -1 ? args[branchIdx + 1] : null,
          slug: slugIdx !== -1 ? args[slugIdx + 1] : null,
        }, output, error, raw, healthModule.runQuickHealthCheck);
      } else if (subcommand === 'remove') {
        worktreeModule.cmdWorktreeRemove(cwd, args[2], output, error, raw, healthModule.runQuickHealthCheck);
      } else if (subcommand === 'get') {
        worktreeModule.cmdWorktreeGet(cwd, args[2], output, error, raw);
      } else if (subcommand === 'list') {
        worktreeModule.cmdWorktreeList(cwd, output, error, raw, healthModule.runQuickHealthCheck);
      } else if (subcommand === 'status') {
        worktreeModule.cmdWorktreeStatus(cwd, output, error, raw);
      } else {
        error('Unknown worktree subcommand. Available: init, add, remove, get, list, status');
      }
      break;
    }
    ...
    case 'lock': {
      ...
      if (subcommand === 'record') {
        worktreeModule.cmdLockRecord(cwd, args[2], output, error, raw);
      } else if (subcommand === 'clear') {
        worktreeModule.cmdLockClear(cwd, args[2], output, error, raw);
      } else if (subcommand === 'check') {
        worktreeModule.cmdLockCheck(cwd, args[2], output, error, raw);
      } else if (subcommand === 'list') {
        worktreeModule.cmdLockList(cwd, output, error, raw);
      } else if (subcommand === 'stale') {
        worktreeModule.cmdLockStale(cwd, args[2], output, error, raw);
      } else {
    ...
    case 'health': {
      ...
      if (subcommand === 'check') {
        ...
        healthModule.cmdHealthCheck(cwd, { ageThreshold }, output, raw);
