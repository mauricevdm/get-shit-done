...
function cmdWorktreeInit(cwd, raw) {
...
function cmdWorktreeAdd(cwd, phase, worktreePath, options, raw) {
  if (!phase) {
...
function cmdWorktreeRemove(cwd, phase, raw) {
  if (!phase) {
...
function cmdWorktreeGet(cwd, phase, raw) {
  if (!phase) {
...
function cmdLockRecord(cwd, phase, raw) {
  if (!phase) {
...
function cmdLockClear(cwd, phase, raw) {
  if (!phase) {
...
function cmdLockCheck(cwd, phase, raw) {
  if (!phase) {
...
function cmdLockStale(cwd, phase, raw) {
  if (!phase) {
...
async function main() {
  ...
  switch (command) {
    ...
    case 'worktree': {
      const subcommand = args[1];
      if (subcommand === 'init') {
        cmdWorktreeInit(cwd, raw);
      } else if (subcommand === 'add') {
        const branchIdx = args.indexOf('--branch');
        const slugIdx = args.indexOf('--slug');
        cmdWorktreeAdd(cwd, args[2], args[3], {
    ...
    case 'lock': {
      ...
      if (subcommand === 'record') {
        cmdLockRecord(cwd, args[2], raw);
      } else if (subcommand === 'clear') {
        cmdLockClear(cwd, args[2], raw);
      } else if (subcommand === 'check') {
        cmdLockCheck(cwd, args[2], raw);
      } else if (subcommand === 'list') {
        cmdLockList(cwd, raw);
